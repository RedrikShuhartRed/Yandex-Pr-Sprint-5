...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	SpeedShift := CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift
	calories := SpeedShift * r.Weight / MInKm * r.Duration.Hours() * MinInHours
	return calories
}
...
func (w Walking) Calories() float64 {
	...
	HeightMultiplier := CaloriesSpeedHeightMultiplier * w.Weight
	SpeedMultiplier := math.Pow(w.meanSpeed()*KmHInMsec, 2) / (w.Height / CmInM)
	calories := (WeightMultiplier + SpeedMultiplier*HeightMultiplier) * w.Duration.Hours() * MinInHours
	return calories
}
...
func (s Swimming) meanSpeed() float64 {
	...
	meanSpeed := float64(s.LengthPool*s.CountPool) / MInKm / s.Duration.Hours()
	return meanSpeed
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	MeanSpeedShift := s.meanSpeed() + SwimmingCaloriesMeanSpeedShift
	WeightMultiplier := SwimmingCaloriesWeightMultiplier * s.Weight
	calories := MeanSpeedShift * WeightMultiplier * s.Duration.Hours()
	return calories
}
